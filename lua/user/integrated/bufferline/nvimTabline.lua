...
local function has_text(segment)
  return not (segment == nil or segment.text == nil or segment.text == "")
end
...
local function get_component_size(segments)
  ...
  for _, segment in pairs(segments) do
    if has_text(segment) then
      sum = sum + strwidth(tostring(segment.text))
    end
  end
  return sum
end
...
function M.apply()
  require("bufferline.ui").tabline = function(items, _)
    ...
    local tab_indicator_segments = {}
    local tab_indicator_length = require("tabline").total_tab_length
